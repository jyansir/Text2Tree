...
def one_hot(labels, num_labels):
    ...
    one_hot_labels = torch.eye(num_labels, device=labels.device, dtype=torch.float32)
...
class ContrastModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            if labels.ndim == 1:
                loss_fct = CrossEntropyLoss() # we can add high level label as empty label and still use CE
                target = labels.view(-1)
                labels = one_hot(target, self.num_labels)
            else:
                # loss_fct = F.binary_cross_entropy_with_logits
                loss_fct = nn.BCEWithLogitsLoss()
                target = labels.to(torch.float32)
                if num_used_labels is not None:
                    target = target[:, -num_used_labels:]
            ...
            if self.cls_loss:
                if self.num_labels == 1:
                    ...
                    if num_used_labels is not None:
                        loss += loss_fct(logits.view(-1, self.num_labels)[:, -num_used_labels:], target)
                    else:
            ...
            if self.training:
                ...
                if num_used_labels is not None:
                    loss += loss_fct(contrast_logits.view(-1, self.num_labels)[:, -num_used_labels:], target)
                else:
