...
def convert_hpt_data(dataset, data_path):
    @dataclass
    class DataArgs:
        dataset: str
    data_args = DataArgs(dataset)
    ...
    torch.save(new_valuedict, f"{data_path}/hpt_value_dict.pt")

    # parent node (label) number (non-leaf labels)
    with open(f"{data_path}/hpt_data_info.json", 'w') as f:
    ...
    torch.save(slot2value, f"{data_path}/hpt_slot.pt")
    ...
    def write_data(Xs, spl, dataset):
        ...
        with open(f'{data_path}/hpt_{spl}.json', 'w') as f:
