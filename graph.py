# https://github.com/wzh9969/HPT/blob/main/models/graph.py
import torch
...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph_type='GAT', layer=1, path_list=None, data_path=None, n_leaf=-1):
        super(GraphEncoder, self).__init__()
        # self.n_leaf = n_leaf
        self.register_buffer('n_leaf', torch.tensor(n_leaf).long())
        self.config = config
        ...
        if self.graph_type == 'graphormer':
            ...
            new_path_list = {x[1]: x[0] for x in path_list}
            childs = [x[1] for x in path_list]
    ...
    def forward(self, label_emb, embeddings):
        ...
        if self.graph_type == 'graphormer':
            ...
            label_emb = label_emb[3:] + (label_name_emb * label_mask.unsqueeze(-1)).sum(dim=1) / label_mask.sum(dim=1).unsqueeze(-1)

            label_emb = label_emb[3:] + self.id_embedding(self.label_id[:, None]).view(-1,
        ...
        return label_emb.squeeze(0)[-self.n_leaf:]
